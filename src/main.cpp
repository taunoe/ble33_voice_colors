...
/* Includes ---------------------------------------------------------------- */
#include "PDM.h"
#include "Responding-to-color-name_inferencing.h"
  ...
  if (++print_results >= (EI_CLASSIFIER_SLICES_PER_MODEL_WINDOW)) {
    ...
    for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
      if (result.classification[ix].value > THRESHOLD) {
        ...
        if (ix == 0) {
          digitalWrite(Punane_pin, HIGH);
          digitalWrite(Sinine_pin, LOW);
          digitalWrite(Roheline_pin, LOW);
        }
        if (ix == 1) {
          digitalWrite(Punane_pin, LOW);
          digitalWrite(Sinine_pin, LOW);
          digitalWrite(Roheline_pin, HIGH);
        }
        if (ix == 2) {
          digitalWrite(Punane_pin, LOW);
          digitalWrite(Sinine_pin, HIGH);
          digitalWrite(Roheline_pin, LOW);
        }
      }
    }
